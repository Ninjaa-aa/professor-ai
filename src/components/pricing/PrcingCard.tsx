...
export const PricingCard: React.FC<PricingCardProps> = ({ plan }) => {
  return (
    <div 
      className={`relative overflow-hidden rounded-3xl p-1 group hover:scale-105 transition-transform duration-300
        ${plan.highlight ? 'bg-gradient-to-b from-blue-500/20 via-purple-500/20 to-transparent' : ''}`}
    >
      <div className="absolute inset-0 bg-gradient-to-b from-blue-500/20 via-purple-500/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <div className="relative h-full rounded-[2.5rem] bg-gradient-to-b from-slate-900/90 to-black p-8 flex flex-col backdrop-blur-xl">
        ...
        <div className="flex-grow">
          <div className="space-y-4 mb-8">
            {plan.features.map((feature: { included: boolean; name: string }, index: number) => (
